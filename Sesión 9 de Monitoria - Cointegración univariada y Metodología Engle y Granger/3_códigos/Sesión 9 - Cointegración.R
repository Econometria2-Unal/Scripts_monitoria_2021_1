...
summary(ur.df(TB3MS, lags=6, selectlags = "AIC", type = "trend"))
...
summary(ur.df(TB3MS, lags=6,selectlags = "AIC", type = "drift"))
...
#Esta es la correcta especificaci?n de la prueba dado que los términos determinísticos no son significativos. 
summary(ur.df(TB3MS,lags=6, selectlags = "AIC", type = "none"))
...
summary(ur.kpss(TB3MS, use.lag = 6, type = "mu")) #el mu nos indica que es una prueba sin términos determinísticos.
...
#El tau me dice si la serie tiene o no al menos una raíz unitaria. El phi3 me dice si la tendecia es 
#significativa o no, y por tanto, si el el test de raíz unitaria debería incluir o no tendencia. 
summary(ur.df(TB10YS,lags=6, selectlags = "AIC", type = "trend"))
#El tau nos dice que la serie tiene al menos una raíz unitaria, mientras el phi3 nos dice que la tendencia 
#no es significativa. Por lo tanto, vamos a mirar únicamente la prueba con intercepto.

#El tau me dice si la serie tiene o no al menos una raíz unitaria. El phi2 me indica si la deriva es
#significativa, y por consiguiente, si se debe incluir en el test de raíz unitaria. 
summary(ur.df(TB10YS, lags=6,selectlags = "AIC", type = "drift"))
#Los resultados indican que la serie tiene al menos una raíz unitaria. El phi2, por su parte, indica que 
...
summary(ur.df(TB10YS, lags=6, selectlags = "AIC", type = "none"))
#Claramente se evidencia que la serie tiene al menos una raíz unitaria, en tanto no se rechaza la hipótesis nula. 
#Noten la importancia de determinar si la serie tiene términos detemrminísticos, pues el valor calculado en cada
#especificación de la prueba cambió de forma importante.

#Hacemos la prueba KPSS cuya hipótesis nula es que la serie es estacionaria, la cual es totalmente contraria a la ADF. 
summary(ur.kpss(TB10YS, use.lag = 5, type = "mu")) #el mu nos indica que es una prueba sin términos determinísticos.
...
summary(ur.df(diff(TB10YS), lag=6, selectlags = "AIC", type = "none")) #Estacionaria
summary(ur.df(diff(TB3MS), lag=6, selectlags = "AIC", type = "none")) #Estacionaria. 
...
summary(ur.df(z_hat,lags=6,type = "none", selectlags = "AIC")) #Los residuales son estacionarios: hay cointegración. 
...
#a pesar de que las mecánica sea lo misma.  Lo anterior, se debe a que los residuales fueron estimados (no son los verdaderos errores porque no 
# son conocidos) debido a que  el vector de cointegración (el theta) en el caso univariado tuvo que ser estimado en la primer etapa
...
# En este caso decidimos usar solo 1 rezago
...
# De 200 datos Y 2 variabales en la relación de cointegración el test rechaza la hipotesis nula 
# de no cointegracion a valores de significancia del  5% pero no del 1%. Como en economia se usa generalmente un 
#nivel de significancia del 5 % se concluye de que hay coingegracion entre TB10YS y TB3MS
...
# de la primera diferencia de TB3MS. -1 esta asociado con el adelantio mientras que 1 con el rezago
...
summary(ur.df(residuals(FS_MCOD6), lag=6, type = "none", selectlags = "AIC")) #Residuales estacionarios,
...
summary(ur.df(residuals(VEC_EQ1), lag=6, type = "none", selectlags = "AIC")) #Residuales estacionarios.
summary(ur.df(residuals(VEC_EQ2), lag=6, type = "none", selectlags = "AIC")) #Residuales estacionarios.
...
# Test Box-Pierce para autocorrelación en los residuales
Box.test(residuals(VECM_EQ1),lag=60, type = c("Box-Pierce")) #NO rechazo H0, se cumple el supuesto. 
Box.test(residuals(VECM_EQ1),type='Box-Pierce',lag=20) #NO rechazo H0, se cumple el supuesto. 
Box.test(residuals(VECM_EQ1),type='Box-Pierce',lag=30) #NO rechazo H0,  se cumple el supuesto.

Box.test(residuals(VECM_EQ2),lag=60, type = c("Ljung-Box")) #NO rechazo H0, se cumple el supuesto. 
Box.test(residuals(VECM_EQ2),type='Ljung-Box',lag=20) #NO rechazo H0, se cumple el supuesto. 
Box.test(residuals(VECM_EQ2),type='Ljung-Box',lag=30) #NO rechazo H0,  se cumple el supuesto.
...
#El tau me dice si la serie tiene o no al menos una raíz unitaria. El phi3 me dice si la tendecia es 
#significativa o no, y por tanto, si el el test de raíz unitaria debería incluir o no tendencia. 
summary(ur.df(e, lags=6, selectlags = "AIC", type = "trend"))
#El tau nos dice que la serie tiene al menos una raíz unitaria, mientras el phi3 nos dice que la tendencia 
#no es significativa. Por lo tanto, vamos a mirar únicamente la prueba con intercepto.

#El tau me dice si la serie tiene o no al menos una raíz unitaria. El phi2 me indica si la deriva es
#significativa, y por consiguiente, si se debe incluir en el test de raíz unitaria. 
summary(ur.df(e, lags=6, selectlags = "AIC", type = "drift"))
#Los resultados indican que la serie tiene al menos una raíz unitaria. El phi2, por su parte, indica que 
#la deriva de la serie no es significativa, por tanto, debe hacerse una prueba sin términos determinísticos.

#Esta es la correcta especificación de la prueba dado que los términos determinísticos no son significativos. 
summary(ur.df(e, lags=6,selectlags = "AIC", type = "none"))
#Claramente se evidencia que la serie tiene al menos una raíz unitaria, en tanto no se rechaza la hipótesis nula. 
#Noten la importancia de determinar si la serie tiene términos detemrminísticos, pues el valor calculado en cada
#especificación de la prueba cambió de forma importante.

#Hacemos la prueba KPSS cuya hipótesis nula es que la serie es estacionaria, la cual es totalmente contraria a la ADF. 
summary(ur.kpss(e, use.lag = 2, type = "mu")) #el mu nos indica que es una prueba sin términos determin?sticos.
#Claramente rechazamos la hipótesis nula de estacionariedad, así que las serie no es estacionaria. 
...
#El tau me dice si la serie tiene o no al menos una raíz unitaria. El phi3 me dice si la tendecia es 
#significativa o no, y por tanto, si el el test de raíz unitaria debería incluir o no tendencia. 
summary(ur.df(lp.j, lags=6, selectlags = "AIC", type = "trend"))
#La serie según esta prueba es estacionaria en torno a una tendencia determií?stica. 

#Hacemos la prueba KPSS cuya hipótesis nula es que la serie es estacionaria, la cual es totalmente contraria a la ADF. 
summary(ur.kpss(lp.j, use.lag = 6, type = "tau")) #el tau nos indica que es una prueba con términos determinísticos.
#Claramente rechazamos la hipótesis nula de estacionariedad, así que las serie no es estacionaria. Nos inclinamos por
#esta prueba al evaluar la gráfica de la serie. 


# Prueba sobre el logaritmo del nivel nacional de precios

#Primero vamos a hacer una prueba de ADF con tendencia e intercepto.Los valores críticos de la prueba son sensibles a 
#la inclusión de términos determin?sticos, de manera que hay que determinar si la serie tiene tendencia y/o intercepto. 

#El tau me dice si la serie tiene o no al menos una raíz unitaria. El phi3 me dice si la tendecia es 
#significativa o no, y por tanto, si el el test de raíz unitaria debería incluir o no tendencia. 
summary(ur.df(lp.u,lags=6, selectlags = "AIC", type = "trend"))
...
summary(ur.kpss(lp.u, use.lag = 2, type = "tau")) #el tau nos indica que es una prueba con términos determinísticos.
#Claramente rechazamos la hipótesis nula de estacionariedad, así que las serie no es estacionaria. 
...
autoplot(lp.j, col="red", main = "logaritmo del indice de precios de Jap?n",xlab = "Fecha", ylab="", lwd=2)
autoplot(lp.u, col="red", main = "logaritmo del indice de precios de US",xlab = "Fecha", ylab="", lwd=2)
...
#Haremos la prueba ADF sin términos determinísticos.
summary(ur.df(res, lags=6,type = "none", selectlags = "AIC")) #Los residuales son estacionarios: hay cointegración. 
...
#Analizamos que los residuales sean estacionarios.
summary(ur.df(residuals(VEC_EQ1), lags=6, type = "none", selectlags = "AIC")) #Residuales estacionarios.
summary(ur.kpss(residuals(VEC_EQ1), type="mu", use.lag = 5)) #Residuales estacionarios.
...
#MCOD, es natural que no se satisfaga el supuesto, por lo cual se utilizaría errores tipo HAC. Para VECM es necesario introducir rezagos adicionales, sobre
