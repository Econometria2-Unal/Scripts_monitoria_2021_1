...
#                 SESIÓN #2 MONITORIA : ESTADÍSTICA BÁSICA
##______________________________________________________________________________
##______________________________________________________________________________
...
#Importción de datos usando las biblioteas de tidyverse----------------------------------
...
titanic2 = read_csv(file.choose()) # La función read_csv es diferente a read.csv
...
help(mean) #na.rm es para que las casillas en las que no hay información no se tomen en cuenta
...
#Barras barplot
x11()
barplot(table(Sex), main ="Frecuencia de género", xlab="Género", ylab ="Número", col="brown")
...
#del consumo de alcohol, el sexo del bebé, la edad de la madre y de la raza de la madre.
MODELO1 = lm(bwght ~ cigs + drink + npvis + male + mage + mwhte + mblck, data = bwght2)
summary(MODELO1)

#En esta regresión, a parte de las anteriores variables, controlamos por la educación del padre y de la madre, pues
#es posible que al no incluirlas en la regresión se presente un sesgo de variable omitida: ¿Puede haber correlación 
...
#A partir del mismo argumento utilizaremos el promedio de la educación de ambos padres, con objeto de reducir las contradicciones
...
## title: introducir un título
## type: el formato de la tabla (e.g. text o latex)
## omit: si se quiere omitir alguna variable de la tabla
## style: específica el estilo de la tabla (e.g. aer para American Economic Review)
...
#Presentación de resultados
library(stargazer)
stargazer(RegresiónA1, RegresiónA2, type="text", column.labels = c("N-N","N-L"),keep.stat = c("n", "rsq" "adj.rsq", "aic"))
...
fitted.values(RegresiónA2)

#Residuales de la regresión
residuals(RegresiónA2)
rstandard(RegresiónA2) #Residuales estandarizados (divididos por su desviación estándar)
...
#Erróres robustos a la heterocedasticidad y correlación con HAC
install.packages("sandwich")
library("sandwich")

vcovHC(RegresiónA2)
coeftest(RegresiónA2, vcov=vcovHC(RegresiónA2))
coeftest(RegresiónA2)
...
install.packages("car")
library(car)
install.packages("patchwork") #Con este paquete podemos unificar gráficas del mismo formato
library(patchwork)
...
#Ahora graficamos utilizando la función qqplot
