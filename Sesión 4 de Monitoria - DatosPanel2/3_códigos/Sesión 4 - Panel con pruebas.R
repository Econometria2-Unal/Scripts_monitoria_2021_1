...
#                 SESIÓN #4 MONITORIA : Modelos para datos panel.
##______________________________________________________________________________
##______________________________________________________________________________
...
# install.packages("stargazer")   #Tablas m?s est?ticas 
...
#Base de datos: Wooldridge 14.4 ?Ha cambiado la educaci?n a lo largo del tiempo?
...
plotmeans(lwage ~ married, main="Distribuci?n de la poblaci?n", data=panel.Education)

#Definici?n de las variables
exper2 = expersq
panel.Education$yr = factor(panel.Education$year)#Una Dummy para cada a?o de la muestra

#Estimaci?n MCO combinados -E.F.- E.A.----------------------------------------------------

#M?nimos cuadrados combinados.
...
## El modelo between en plm realiza una transformaci?n diferente a la que vieron  
## en clase, pues tiene en cuenta las variaciones que puedan existir entre grupos 
## de individuos. Una anotaci?n importante es que con este tipo de modelos se
...
## usa la media de todos los datos para un a?o especifico.
...
#La prueba no es concluyente, tanto los err?res diferenciados como sin diferenciar tienen correlaci?n serial. 
...
#subsidio a la capacitaci?n laboral en las horas de capacitaci?n por empleado. El modelo 
#b?sico para los tres a?os es: 
