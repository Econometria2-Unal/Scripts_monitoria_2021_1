...
### Para revisar más sobre el paquete broom
vignette("broom") 

### Para revisar que funciones son compatibles con el paquete broom
...
# Data frame con info. adicional sobre la regresióin
glance_mpl = glance(MPL); View(glance_mpl)  

# Data frame de datos expandido con desviacione estándar poblacional estimada, residuales, fitted values y m?s 
...
# Acuerdense de esta predicción porque va a ser usada también para los modelos probit y logit 
...
# que es la probabilidad de exito para los diferntes valores z_ajustados en el modelo para un modelo logit
...
### qué tanto es más probable que se de el evento de éxito cuando ocurre un aumento en 1 unidad 
### para un determinado regresor mantienendo los demás regresores fijos
...
#### 1. Cálculo el promedio de todas las variable o la moda (dependiendo del tipo de variable que estoy 
####    trabajando)
#### 2. Frente a dicho resultado cálculo el efecto marginal 
...
p_1_norm = pnorm(0.270 - 0.012 * mean(nwifeinc) + 0.131 * mean(educ) 
             + 0.123 * mean(exper) - 0.0019 * mean(expersq) - 0.053 * mean(age) + 0.036 - 0.868 )
...
margins(LOGIT, type = "response", data.frame(nwifeinc=mean(nwifeinc), educ = mean(educ), 
                                             age = mean(age), exper = mean(exper), kidslt6 = 1, kidsge6 = 1))
margins(PROBIT, type = "response", data.frame(nwifeinc=mean(nwifeinc), educ = mean(educ), 
                                             age = mean(age), exper = mean(exper), kidslt6 = 1, kidsge6 = 1))
